...
def run(ref,samplelist,fastq_dir,out_dir,normal="false"):
    ...
    for (root,dirs,files) in os.walk(fastq_dir):
        for file in files:
            ...
            cmd += " --cnv-enable-self-normalization false "
...
if __name__=="__main__":
    if len(sys.argv)==6:
        ...
        print("\npython3 %s /ref/ /path/to/sample.list /path/to/fastq-dir/ /path/to/outdir/ /path/to/PoN.list\n" % (sys.argv[0]))
        print("\npython3 %s /ref/ /path/to/sample.list /path/to/fastq-dir/ /path/to/outdir/\n" % (sys.argv[0]))
