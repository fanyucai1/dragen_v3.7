...
if args.wes:
    ...
    #fastq2vcf
    cmd='%s %s/core/germline_fastq2vcf.py -r %s -s %s -o %s -f %s -p %s -b %s'%(python3,dir_name,args.ref,args.wes,wes_vcf,fastq_dir,args.normal_wes,args.bed)
...
if args.wgs:
    ...
    # fastq2vcf
    cmd = '%s %s/core/germline_fastq2vcf.py -r %s -s %s -o %s -f %s -p %s' % (python3, dir_name, args.ref, args.wgs, wgs_vcf, fastq_dir, args.normal_wgs)
    subprocess.check_call(cmd, shell=True)
