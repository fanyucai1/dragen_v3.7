...
for i in range(0,len(samplename)):
    if os.path.exists("%s.time_metrics.csv" % (args.out_dir + "/" + samplename[i] + "/" + samplename[i])):
        ...
        if args.PoN != "false":
            cmd += " --cnv-normals-list %s --cnv-enable-self-normalization false --cnv-enable-gcbias-correction false " % (
                args.PoN)
        else:
            cmd += " --cnv-enable-self-normalization true --cnv-enable-gcbias-correction true "
        if args.bed != "false":
            cmd += " --sv-exome true --vc-target-bed %s --cnv-target-bed %s --sv-call-regions-bed %s --cnv-interval-width 1000 " % (
            args.bed, args.bed, args.bed)
        else:
